...
<head>
  ...
  <style>
    ...
    /* Tambahan style untuk menu aktif */
    .menu-item-wrapper.active {
      background-color: #d1cce3; /* Highlight warna */
      border-radius: 6px;
    }
  </style>
</head>
<body class="bg-white">
  <div class="min-h-screen flex flex-col">
    ...
    <div class="flex flex-1">
      <nav id="sidebar" class="sidebar bg-[#9b95cc] flex flex-col items-start py-8 px-1 space-y-8 border border-black border-r-0">
        ...
        <a href="data_guru.php" class="menu-item-wrapper flex items-center space-x-2 text-black text-sm font-normal cursor-pointer select-none px-2 w-full">
  ...
  <script>
    ...
    // Menandai menu sidebar yang aktif saat diklik
    const menuItems = document.querySelectorAll('.menu-item-wrapper');

    menuItems.forEach(item => {
      item.addEventListener('click', function () {
        // Hilangkan aktif di semua menu
        menuItems.forEach(i => i.classList.remove('active'));
        // Tambah aktif di menu yang diklik
