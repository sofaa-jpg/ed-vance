...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Simpan ke database dengan role siswa
    $sql = "INSERT INTO users (username, pass, nis, jenis_kelamin, nama_lengkap, no_telp, foto, role)
            VALUES ('$username', '$password', '$nis', '$jenis_kelamin', '$nama', '$telepon', '$foto_path', 'siswa')";

    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Pendaftaran berhasil!'); window.location.href='login_siswa.php';</script>";
    } else {
...
<body class="bg-white">
  <header class="bg-[#2f2f6e] flex items-center px-6 py-4">
    <h1 class="text-white font-extrabold text-2xl drop-shadow-[2px_2px_0_rgba(0,0,0,0.5)]">
      ED-VANCE <i class="fas fa-book-open text-white text-4.3xl"></i>
    </h1>
  </header>
  <main class="p-6 max-w-full overflow-y-auto h-[calc(100vh-72px)]">
    <h2 class="font-extrabold text-xl mb-2 border-b border-black pb-1">
      Pendaftaran Akun
    </h2>
    ...
    <form class="space-y-4 text-xs w-full max-w-full" method="POST" enctype="multipart/form-data">
      <div>
        <label class="block mb-1" for="nis">NIS*</label>
        <input class="w-full border border-gray-300 rounded-md px-3 py-1 text-xs" id="nis" name="nis" type="text" required/>
      </div>
      <div>
      ...
      <div>
        ...
        <select class="w-full border border-gray-300 rounded-md px-3 py-1 text-xs text-gray-700" id="jenis_kelamin" name="jenis_kelamin" required>
